      ...
      <swiper class="index_swiper" :options="swiperOption" ref="boxSwiper">
        ...
        <swiper-slide class="index_swiper_fir">
          ...
          <div :class="{ on: frontend.firOn == 'on2' }">
            <swiper class="index_swiper_banner" :options="bannerOption" ref="bannerSwiper">
              <swiper-slide v-for="(item, index) in 1" :key="'swiper' + index"><img src="@/assets/images/index_1_img01.png"
                  alt="" /></swiper-slide>
        ...
        <swiper-slide class="index_swiper_four" :class="{ on: frontend.fourOn }">
          ...
          <div class="list">
            ...
            <div class="li" v-for="(item, index) in frontend.fourList" :class="{ on: frontend.fourListIndex == index }"
              @click="changeFourList(index)">{{ item }}</div>
          </div>
...
<script>
  ...
  export default {
    ...
    head() {
      return {
        title: this.interface.SEOInfo.seo_title,
        meta: [{
            hid: 'keywords',
            name: 'keywords',
            content: this.interface.SEOInfo.seo_keyword
          },
          {
            hid: 'description',
            name: 'description',
            content: this.interface.SEOInfo.seo_desc
          }
        ]
      }
    },
    ...
    created() {
      ...
      this.$admin_base([
        ...
        (res) => {
          ...
          this.interface.SEOInfo = res.data
        }
      ])
    },
    mounted() {},
