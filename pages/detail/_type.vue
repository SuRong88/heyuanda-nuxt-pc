<template>
    <div class="about-page about-detail">
        ...
        <div class="content wrapper clear">
            <div class="con-l fl">
                <div class="l-head">
                    <div class="date" v-if="detail.create_time">{{ detail.create_time | formatDate('yyyy年MM月dd日') }}</div>
                    <h2 class="tit">{{ detail.name }}</h2>
                </div>
                <div class="content-main edit_textBox" v-html="detail.body"></div>
            </div>
            <div class="con-r fr">
                <div class="r-head"><h3 class="tit">文章推荐</h3></div>
                <!-- 推荐 -->
                <ul class="recom-box" v-if="detail.recommend && detail.recommend.length > 0">
                    <nuxt-link
                        ...
                        class="recom-item"
                    >
                        <p class="recom-date">{{ item.create_time | formatDate('yyyy年MM月dd日') }}</p>
                        <p class="recom-tit">{{ item.name }}</p>
                    </nuxt-link>
                </ul>
            </div>
        </div>
...
export default {
    ...
    async asyncData({ route, params }) {
        ...
        let id = params.type.split('-')[1];
        let [Res] = await Promise.all([
            axios.get(URL.getArticleDetail, {
                params: {
                    id: id
                }
            })
        ...
        return {
            type: type,
            bannerTitleCn: type == 'company' ? '公司动态' : '行业动态',
            bannerTitleEn: type == 'company' ? 'Company news' : 'Industry news',
